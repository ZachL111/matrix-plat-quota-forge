...
`matrix-plat-quota-forge` is centered on platform engineering work in Java. The technical target is: Package a Java local lab for quota analysis with transition tables, invalid-transition tests, and documented operating limits.
...
The score is deterministic. The current threshold is 165. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 2.
